...
- Require Node.js 22.22.3+/24.15.0+/25.9.0+ and OpenClaw 2026.8.1-beta.1+.
...
- Verify `openclaw.plugin.json`, `package.json#openclaw.extensions`, the Skill, and the manifest MCP mapping.
...
- Confirm the packaged manifest and any operator-level override both send the non-secret telemetry header `X-Kling-Integration: Plugin-OpenClaw`; a manual generic MCP connection without this header remains distinguishable.
